...
- [x] Municipal report **"Include child boundaries"** toggle (D3 default OFF) — folds child MPA/hotspot/custom events/patrols into the municipal report; reflected in PDF export + on-screen figures — **DONE 2026-07-09** (branch `feat/boundaries-phase4b-include-children`, off Phase 4A, DEV-only LOCAL/unpushed). Foundation `municipality-scope.ts`: `resolveChildZoneIds(tenantId, muniIds)` (protectedZone.parentMunicipalityId → zone ids) + `buildMunicipalityScopeWhere(muniIds, childZoneIds?)` — plain `municipalityId` normally; widens to `{OR:[{municipalityId},{coveredZones:{some:{protectedZoneId:{in:childZoneIds}}}}]}` when the municipality has child zones. `includeChildren?: boolean` threaded through reportMap (7 procedures) + map (events + patrol-tracks) + PDF (`parseReportMapParams`/report-data/reportExport) + filter context/bar (shadcn Switch shown ONLY for a specific municipality, cleared by province/reset). Gate GREEN (check-product-sync · typecheck 7/7 · turbo lint 6/6 · web build · vitest web 1717/shared/jobs · audit exit 0, 1 pre-existing moderate). Dev Visual QA vs SQL EXACT: Sablayan→Apo Reef OFF **73** → ON **106** (+33) ✅ live in the UI; Calapan City→Harka Piloto +14 ✅ (SQL OR-clause replication matched exactly); toggle hidden for All/province; 0 console errors.
- [x] Verify province/municipal aggregations across dashboard + PDF report paths — **DONE 2026-07-09**. Province aggregations verified vs SQL on the report map + PDF path (Phase 4A). The `municipalityCoverage` chart — the ONE report-map panel still province-blind — is now province-narrowed: `province?` added to the `municipalityCoverage` procedure (reuses the shared `resolveMunicipalityScope`; municipalityId still wins over province; municipality list + patrol/event group-bys all scope to the province's munis), threaded from the report map's existing Province `<Select>` via `report-map-view.tsx`. No standalone dashboard control needed — the chart renders on the report map, which already has the province filter. +2 procedure tests; gate GREEN (typecheck 7/7 · turbo lint 6/6 · web build · vitest 1719). *(A dedicated Command-Center-wide province filter across every KPI remains a separate, larger optional effort — not part of this item.)*
