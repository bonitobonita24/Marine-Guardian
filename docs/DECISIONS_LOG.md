...
## Custom-role matrix layer — data-driven RBAC below the 3 fixed tiers (2026-07-11)
Decision: Custom-role matrix layer — data-driven RBAC below the 3 fixed tiers.
Custom-role users are anchored at role=tenant_admin (enum unchanged) + nullable
User.customRoleId FK; permissions resolved from role_permissions matrix (strict
CRUD: view/write/update/delete), deny-by-default, hard-clamped to ≤ tenant_admin
ceiling, NEVER granting Billing or User Management. Feature granularity =
per-nav-href. Tenant-scoped; only tenant_superadmin (+ platform tenant_manager)
may build/assign. LOCAL build only — deploy owner-gated.
Files: packages/db/prisma/schema.prisma (CustomRole + RolePermission models,
User.customRoleId, Tenant.customRoles), packages/db/prisma/migrations/
20260711090000_add_custom_role_permissions/migration.sql.
Locked: yes
...
- [swarm S8 · 2026-07-01 13:06:16] S8/q-S8-02: A/high — approved follow-up to extract MapReadySignal (double-rAF + __renderPending + 8s timeout) to shared components/map-ready-signal.tsx; refactor 4 call sites (event-points-map, patrol-tracks-map, area-coverage-map, per-area-heatmap-map). Rationale: DRY on load-bearing Puppeteer-sync protocol; prevents 4-way drift. Route as follow-up backlog item, not inline S8 scope.
- [swarm S8 · 2026-07-01 13:06:54] S8/q-S8-03: A — fix hardcoded 'Blue Alliance logo' alt in report-map-report.tsx:127 by using role-based generic alt ('Partner organisation logo' / 'Municipal logo'). Satisfies WCAG 2.2 AA SC 1.1.1 per PRODUCT.md L377/L430; no ReportTemplate schema change in this session (deferred as future product decision).
