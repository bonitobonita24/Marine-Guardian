...
- [ ] Sub-batch 5.1b: applyAreaDerivation persistence helper — apps/web/src/server/sync/area-derivation.ts loads row+tenant enabled boundaries, calls 5.1a deriveArea, writes back areaBoundaryId + areaDerivedAt, logs matchedVia to AuditLog per v2 L561 audit requirement; vitest with mocked prisma.
- [ ] Sub-batch 5.1c: BullMQ area-rederive queue + processor + worker — packages/jobs/src/queues/area-rederive.queue.ts + processors/area-rederive.processor.ts + workers/index.ts + start-workers.ts + queues/types.ts; rate-limited to 50 jobs/sec per v2 L545; processor calls applyAreaDerivation per row.
- [ ] Sub-batch 5.1d: Sync-engine inline re-derive + AreaBoundary CUD enqueue — er-sync.processor.ts inline applyAreaDerivation when Event/Patrol upsert changes areaName per v2 L546; area-boundary router enqueues rederive batch jobs on create/update/delete/enable/disable for all Event+Patrol+FuelEntry rows in tenant per v2 L545.
- [ ] Sub-batch 5.1e: Admin manual-rebuild tRPC mutation + UI button — super-admin-only procedure on area-boundary router that enqueues full-tenant rederive; UI button on /admin/area-boundaries with confirm dialog.
