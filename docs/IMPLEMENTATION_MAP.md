...
# Last updated: 2026-05-12 — Phase 8 Batch 2 Item 3: Alert History Log shipped (migration 20260512044045_add_alert_history). New AlertHistory model + processor wiring + tRPC router + /alerts/history UI page. One row per (rule × event) match, NOT per recipient. Snapshot fields (ruleNameSnapshot, eventTitleSnapshot) survive parent deletion via ON DELETE SET NULL. Spec deferral #1 (PRODUCT.md L182) CLEARED. 57/57 tests pass (was 50/50 — added 2 processor tests + 5 router tests). shadcn Table primitive installed for the history list view (first use in repo). Phase 8 Batch 2 backlog now: PDF/CSV exports + real-time SSE (Tier 3, mandatory split).
# ---
