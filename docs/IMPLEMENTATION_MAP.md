...
### Next Step
SS-3 Alert Rules Export shipped (pending squash-merge from `feat/exports-alert-rules` to main). Phase 8 Batch 2 remaining backlog after Item 4 fully ships:
1. **SS-4 Notifications + Alert History (combined)** — both audit-style views with similar column shapes; route handlers + tests + buttons on /notifications + /alerts/history pages. ~22K. Branch: feat/exports-audit-views. Plan file: `.cline/tasks/phase8-batch2-item4-exports.md` SS-4 section.
...
- ✅ `feat/exports-alert-rules` (SS-3) — Alert Rules Export Route Handler + UI buttons (this branch, squash-merge pending)
