...
# Last updated: 2026-05-12 — Phase 8 Batch 2 Item 4 SS-2: Patrols Export Route Handler shipped on feat/exports-patrols (squash-merge pending). New: apps/web/src/app/api/exports/patrols/route.ts (GET handler — same shape as SS-1 events handler with prisma.patrol.findMany + 10-column PatrolRow: id, serialNumber, title, patrolType, state, boatName, startTime ISO, endTime ISO, totalDistanceKm, createdAt ISO), 6 new route tests (CSV byte-level BOM + tenant scoping, PDF content-type, 401, 413 overflow + no audit row, DATA_EXPORT audit shape with 64-hex filterHash, state+patrolType filter propagation). Modified: patrol.ts router exported `patrolListFilters` z.object (state + patrolType) — single source of truth for filter validation; patrolRouter.list now uses `patrolListFilters.extend({cursor, limit})`. patrols/page.tsx: replaced the stub paragraph with a flex header holding the title + 2-button group (Export CSV + Export PDF, same `<Button asChild><a download href=...>` shape as the events page). Full web suite: 66/66 (was 60), typecheck 6/6 clean, lint 5/5 clean, build registers /api/exports/patrols alongside /api/exports/events as ƒ Dynamic Route. No new lessons — SS-1's BOM-via-arrayBuffer 🟡 fix carried forward verbatim. SS-1 prior: Events Export Route Handler with eventListFilters extract pattern. SS-0 prior: 5 shared helpers under apps/web/src/server/lib/ — route-auth, export-csv (~30-LOC RFC-4180+BOM), export-pdf (React-PDF Document), export-audit (DATA_EXPORT writer), export-filename (`{entity}-{tenant}-{YYYYMMDD-HHmmss}.{ext}`). SS-3 (Alert Rules) and SS-4 (Notifications + Alert History combined) remain; each ~18-22K, independently mergeable. Phase 8 Batch 2 backlog after Item 4 fully ships: 1 item — real-time SSE (Tier 3, mandatory split per memory-governance.md §1).
# ---
...
### Next Step
SS-2 Patrols Export shipped (pending squash-merge). Phase 8 Batch 2 remaining backlog after Item 4 fully ships:
1. **SS-3 Alert Rules export** — `/api/exports/alert-rules/route.ts` + tests + buttons on /alerts/page.tsx (beside "View History" + "New Rule"). Column formatter needs to stringify `notificationChannels` array + `conditionJson` object sensibly. ~18K. Branch: feat/exports-alert-rules.
2. **SS-4 Notifications + Alert History (combined)** — both audit-style views with similar column shapes; route handlers + tests + buttons on /notifications + /alerts/history pages. ~22K. Branch: feat/exports-notifications-history.
...
- ✅ `feat/exports-events` (SS-1, commit 0171309) — Events Export Route Handler + UI buttons merged
- ✅ `feat/exports-patrols` (SS-2) — Patrols Export Route Handler + UI buttons (this branch, squash-merge pending)

No known blockers. All services operational. Main currently at 1c783cd (in sync with origin/main post-SS-1 final).
