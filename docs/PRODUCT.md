...
- **🔴 INCREMENTAL / DELTA-ONLY — HARD REQUIREMENT (owner 2026-06-21, Mindoro `mindoro.pamdas.org`):** Every recurring pull MUST be a **delta** — only records changed/created **after the last checkpoint** up to the scheduled run time. The enqueue path **MUST compute `since` = last successful sync watermark** (max `SyncLog.syncedAt`, or per-record `lastSyncedAt`, per tenant+syncType) and pass it into the job; `er-sync.processor` already threads `since` → `client.getEvents/getPatrols/getObservations(since)` → `?updated_since=<ISO>`. The watermark advances **only on success**. Keeps load tiny on **both** EarthRanger (Mindoro) and our VPS.
  - **⚠ CURRENT BUG to fix as part of this:** `scheduleRecurringErSync`/`enqueueErSync` in `er-sync.queue.ts` build the payload with **NO `since`** → `getEvents()` runs with no query param → **full pull from the beginning**. As-coded, recurring sync would re-pull ALL data every run. This is the exact behavior the owner forbids.
  - **NEVER full-pull in the recurring path** — a `since=undefined` (from-the-beginning) sync is prohibited for scheduled runs because it would **overwrite recent in-app edits** to records. Any one-time initial backfill is a **separate, explicit, admin-only** action AND must still honor the edit-protection merge rule (refresh `erOriginalSnapshot` only; never clobber user-edited fields — see "Editable Records & Edit History").
