...
- **Tactical command-center visual direction (2026-06-26).** The War Room renders in a dark-locked "tactical command center" theme (scoped dark tokens, map-dominant layout) optimized for the 24/7 TV-wall context — independent of the rest of the app's light/dark setting. The redesign is the canonical War Room look-and-feel.
- **KPI sparklines (2026-06-26).** Each top-strip KPI card carries an inline trend sparkline showing the metric's recent movement across the active range, so operators read direction at a glance, not just the current number.
- **Ranger Roster panel (2026-06-26).** The analytics band includes a Ranger Roster summarizing the active ranger force: total active rangers, how many are currently on patrol (linked to an open patrol), and how many are active within the range. Rangers and their patrol links are derived from EarthRanger patrol-segment leaders (KnownRanger / AccompanyingRanger), so the roster reflects real harvested field data.
- **Coverage % headline (2026-06-26).** The War Room surfaces a coverage-percentage headline (patrol-area coverage for the active range) as a first-class command-center metric.
...
**AccompanyingRanger (Command Center native):** id, tenant_id, entity_type (event | patrol), entity_id (references event or patrol), ranger_type (registered | freetext), registered_user_id (nullable — references User if ranger_type=registered), known_ranger_id (nullable — references KnownRanger; populated when the user selects from the autocomplete or after promotion), freetext_name (nullable — for unregistered rangers), added_by_user_id, created_at. The `addAccompanyingRanger` tRPC procedure accepts an optional `knownRangerId` input param (tenant-scoped validation enforced) which links the AR row directly to the KnownRanger registry in a single call.
...
## Background Jobs
- **ER Data Sync (per tenant):** Scheduled job polls EarthRanger API for new/updated events, subjects, patrols, observations, event_types. INCREMENTAL/DELTA-ONLY — events, patrols, observations use `?updated_since=<watermark>` (last successful SyncLog.completedAt); subjects and event_types are full-pull (those endpoints do not support date filtering). Configurable per tenant via admin toggle (recurringEnabled) and interval (intervalMs; default: 300,000ms = 5 min; minimum: 60,000ms = 1 min). Admin can trigger one-shot manual sync via Settings → syncNow. Retry with exponential backoff on failure. SyncLog entry per run. First sync for a tenant (no prior SyncLog) does a full backfill — subsequent syncs are delta-only. erOriginalSnapshot (immutable) captured on first insert.
- **Local Edit + Edit-Protection (M2, REVISION-PRESENCE strategy):** Operators and coordinators can edit Event fields (title, priority, location, offenderName, vesselName, vesselRegistration, address, actionTaken, notesJson, eventDetailsJson) and Patrol fields (title, boatName, areaName) directly in the UI. Each changed field writes an append-only EventRevision/PatrolRevision row (fieldName, beforeJson, afterJson). On subsequent ER syncs, any field with a revision row is SKIPPED in the sync update — local edits survive upstream ER changes (REVISION-PRESENCE conflict default). erOriginalSnapshot remains immutable forever. Edit history UI: newest-first timeline tab on event detail modal and patrol detail page, with ER baseline at the bottom.
- **Settings ER Sync Controls (M2):** Settings page includes a dedicated "EarthRanger Sync" card: recurring-sync enable toggle (admin only, gated on connection verified), sync interval input (60–86400 s, default 5 min), manual "Sync now" button (one-shot delta sync of all 5 types), and last-10 sync log table (type, status badge, records synced, started/completed timestamps).
