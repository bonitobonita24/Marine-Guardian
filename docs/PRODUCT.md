...
- Law enforcement event breakdown: horizontal bar chart by violation type (Unreg Illegal Fishing, Fishing in Prohibited Area, Taking of Prohibited Species, Use of Prohibited Gears, Compressor Fishing, Others, Destructive Practices)
- Monitoring event breakdown: horizontal bar chart by type (Marine Wildlife Sightings, Infrastructure and Assets, Research and Studies, Outreach, Threats on Habitat)
...
- Cross-area comparison table for law enforcement: rows = areas, columns = violation types, cells = counts, with subtotal row
- Cross-area comparison table for monitoring: same structure with monitoring categories
...
- Tabular view of individual event records with all fields: report type, report ID, reported by, reported at, notes, municipality/boundary, violation/event sub-type, vessel name, registration number, address, offender name(s), action taken, photo indicator
...
- Ranger × event type matrix: rows = rangers, columns = each law enforcement type + each monitoring type, cells = count, with subtotal column and row
- Bar chart of total events per ranger (stacked by category)
- Ranger patrol performance table: rows = rangers, columns = foot patrol (count/km/hours) + seabourn patrol (count/km/hours)
...
- Event summary: counts by category
- Foot patrol KPIs: patrols, km, hours
- Seabourn patrol KPIs: patrols, km, hours
- Recent activity timeline: events reported, patrols completed, actions taken
...
Docker Hub:   enabled — hub_repo: bonitobonita24/mpa-command-center
...
Docker Hub publishing: enabled — hub_repo: bonitobonita24/mpa-command-center
