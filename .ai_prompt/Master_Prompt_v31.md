...
>   Full phase details, security rules, and scenarios are in `.claude/rules/` — loaded contextually per task.
...
> - For **Claude.ai chat** (Planning Assistant only): paste this entire file as your first message.
>
...
> ```
> Claude Code        → Primary agent (V31). Planning AND execution via Claude Sonnet 4.6.
...
## PHASE 2.8 — CLICKABLE MOCKUP REVIEW (NEW V31 — Planning Assistant chat only)
**Who:** Planning Assistant chat (Claude.ai) — NOT Claude Code (Cline deprecated V31)
**Where:** The same Claude.ai chat that ran Phases 2 through 2.7
...
**NOTE FOR CLAUDE CODE:** Phase 2.8 does NOT run in your environment. It is
executed by the Planning Assistant chat BEFORE the human brings PRODUCT.md into the
project repo. When Claude Code sees a PRODUCT.md arrive at the project,
...
- An ephemeral single-file HTML mockup with industry-appropriate realistic dummy data
...
- Delivered as Claude.ai artifact + optional downloaded HTML file
...
artifacts. The mockup is a visual alignment tool for the Planning Assistant chat only.
...
- PHASE 2.8 ADDED: New phase in the Planning Assistant chat (Claude.ai) that runs between Phase 2 Alignment Check and Phase 3 handoff. Generates a clickable single-file HTML mockup with industry-appropriate realistic dummy data using shadcn/ui conventions (Tailwind CDN + Inter font + shadcn color tokens). User verifies Claude's interpretation of the spec visually BEFORE Phase 3 locks the architecture.
...
Agent mode:
  ...
  Claude.ai chat    → Planning Assistant only (Phase 2 interview + Phase 2.8 mockup). Not for Phase 3+.
─────────────────────────────────────────────────────────
