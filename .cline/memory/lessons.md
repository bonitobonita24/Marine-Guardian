...
- Files:     apps/web/src/app/(dashboard)/patrol-schedule/_components/__tests__/assignment-dialog.test.tsx L142–146
- Concepts:  react, useEffect, useMemo, dependency-array, infinite-loop, vitest, OOM, shadcn, select, mock, test-infrastructure
- Narrative: MockSelectTrigger is the test mock pattern for shadcn's <SelectTrigger> in jsdom — it accepts test props (testId, id) and writes them into a parent SelectContext via `ctx.setMeta(...)`. The first attempt declared the writing effect as `useEffect(() => { ctx.setMeta({ testId, id }) }, [ctx, testId, id])` which looks correct under exhaustive-deps lint. It is catastrophically wrong. The parent provider holds `meta` as useState and exposes `ctx` via `useMemo(() => ({ meta, setMeta }), [meta, setMeta])`. Every time the effect calls ctx.setMeta() it updates meta, which triggers the useMemo to produce a NEW ctx object reference, which trips the effect's `[ctx, ...]` dep, which re-runs the effect, which calls ctx.setMeta() again — infinite re-render loop. Vitest gives no useful error: it just hangs the worker. Observed symptom: 553-second wall time, 0ms test execution time recorded, FATAL heap exhaustion, no test results emitted. **The fix is to drop `ctx` from the deps array** and rely on the React invariant that `setState` dispatchers (here `ctx.setMeta`, which IS the setState dispatcher even though it's reached via a useMemo'd container) are stable across re-renders. Add an `// eslint-disable-next-line react-hooks/exhaustive-deps` comment with a one-line explanation so the next agent doesn't "fix" it back. **General rule**: whenever an effect's body MUTATES a piece of state that feeds the useMemo computation producing the value you'd otherwise list in deps, list only the primitive prop deps and rely on dispatcher stability. **Reusability**: this MockSelectTrigger pattern is reusable for ANY shadcn Select-based dialog test. The dep-list footgun applies anywhere a memoized context wraps a setState dispatcher and the consumer's effect writes through it. **Cost of the gotcha**: the bug was authored by an interrupted Sonnet dispatch on feat/schedule-conflict-detection mid-2026-05-30; surfaced when the resumed Opus session ran the suite and got the 553-second hang; cost ~10 minutes of diagnostic time before the dep-loop was identified.

## 2026-05-30 — 🔴 exactOptionalPropertyTypes rejects `string | undefined` values in `{prop?: string}` slots — widen the slot type, do NOT type-cast the value
- Type:      🔴 gotcha
- Phase:     Phase 7 Feature Update — Item 5 Schedule Conflict Detection UI tests
- Files:     apps/web/src/app/(dashboard)/patrol-schedule/_components/__tests__/assignment-dialog.test.tsx L96
- Concepts:  typescript, exactOptionalPropertyTypes, strict, optional-properties, type-widening, tsconfig
- Narrative: Marine-Guardian's tsconfig.base.json sets `exactOptionalPropertyTypes: true` (per CLAUDE.md Rule 12). Under this flag, an interface like `{ testId?: string; id?: string }` rejects assignment of a value whose type is `string | undefined`. The optional property is NOT the same shape as a `string | undefined` property — the optional slot accepts "absent" OR `string`; it does NOT accept "present with value undefined". TypeScript surfaces this as TS2375 "Type 'undefined' is not assignable to type 'string' with 'exactOptionalPropertyTypes: true'". **The fix is to widen the property type** explicitly: `{ testId?: string | undefined; id?: string | undefined }`. Do NOT type-cast the value — casting hides the design mismatch and the next refactor will resurface the same error somewhere else. Do NOT remove `exactOptionalPropertyTypes` — it's a project-wide invariant that catches a class of "undefined leaks through an optional" bugs. **When this surfaces in practice**: test mocks that accept React props (where the consumer often passes `someValue || undefined`) frequently hit this. Mock interfaces for component props should always widen optional slots to `T | undefined` because React's prop-passing patterns make `undefined` values common. **Pair with**: dispatcher-stability gotcha above (the SelectMeta type and the MockSelectTrigger deps bug both surfaced in the same test file in the same prior-session ship attempt — addressing them together is the canonical Item-5 UI test scaffolding lesson).

## 2026-05-25 — 🟢 Area Boundary Map Preview shipped — read-only Leaflet dialog opens from new Preview button on /patrol-areas
- Type:      🟢 change
- Phase:     Phase 7 Feature Update — Area Boundary Management UI follow-on (DEFERRED bucket item 1 of 3)
- Files:     apps/web/src/app/(dashboard)/patrol-areas/lib/geojson-to-leaflet-positions.ts NEW (pure helper — accepts Polygon and LineString GeoJSON geometries per Prisma GeometryType enum, validates coordinates shape, flips [lng,lat]→[lat,lng] for Leaflet, returns `{ kind: "Polygon", positions: [number,number][][] }` or `{ kind: "LineString", positions: [number,number][] }` or null for invalid input; colocated under the feature per CLAUDE.md anti-premature-abstraction); apps/web/src/app/(dashboard)/patrol-areas/lib/__tests__/geojson-to-leaflet-positions.test.ts NEW (pure-function vitest coverage, 8 cases: Polygon flip, LineString flip, type mismatch, short LineString, short Polygon ring, missing coordinates, non-finite numbers, non-object input); apps/web/src/app/(dashboard)/patrol-areas/area-boundary-map.tsx NEW (Leaflet island — MapContainer + TileLayer Esri World Imagery satellite + Polygon/Polyline overlay + AutoFitBounds component that calls map.fitBounds on mount/update with 16px padding; scroll-wheel zoom enabled; "use client" Client Component, designed for dynamic import with ssr:false because Leaflet imports window at module load); apps/web/src/app/(dashboard)/patrol-areas/preview-area-boundary-dialog.tsx NEW (shadcn Dialog wrapper — dynamic-imports AreaBoundaryMap with ssr:false to keep the parent page SSR-safe; renders the AreaBoundary name + region in the dialog header; mounts AreaBoundaryMap in dialog body when boundary != null; gracefully renders an emerald "Map cannot be displayed — geometry is malformed" fallback paragraph when geojsonToLeafletPositions returns null instead of crashing the dialog; Close button uses data-testid="preview-close" to disambiguate from shadcn Dialog sr-only Close); apps/web/src/app/(dashboard)/patrol-areas/__tests__/preview-area-boundary-dialog.test.tsx NEW (jsdom vitest — mocks area-boundary-map.tsx to a stub `({boundary}) => <div data-testid="map-mock">{boundary.name}</div>` since vitest jsdom cannot render Leaflet canvas; covers: header rendering, body rendering when boundary present, malformed-geometry fallback message, Close button calls onOpenChange(false); afterEach(cleanup) prevents Radix portal accumulation across cases — matches existing edit-area-boundary-dialog.test.tsx convention); apps/web/src/app/(dashboard)/patrol-areas/area-boundary-table.tsx EDITED (added `onPreview: (boundary: AreaBoundaryRow) => void` prop alongside existing onEdit/onDelete; added Preview button to Actions column BEFORE Edit + Delete buttons, role-gated by isAdmin like the others); apps/web/src/app/(dashboard)/patrol-areas/__tests__/area-boundary-table.test.tsx EDITED (added onPreview prop to all renders to satisfy required-prop typecheck; added "calls onPreview with the row's boundary when Preview is clicked" test case mirroring the existing onEdit/onDelete callback tests; net delta: +1 test case); apps/web/src/app/(dashboard)/patrol-areas/page.tsx EDITED (added `previewTarget: AreaBoundaryRow | null` state mirroring deleteTarget/editTarget pattern; mounts PreviewAreaBoundaryDialog at root level with open={previewTarget != null} + onOpenChange clearing previewTarget; passes onPreview={setPreviewTarget} to AreaBoundaryTable).
- Concepts:  leaflet, react-leaflet, esri, next/dynamic, ssr:false, geojson, shape-validation, [lng,lat]→[lat,lng]
- Narrative: First of three DEFERRED items from the 2026-05-23 Locked Task Queue cleared. Adds a Preview button to the Patrol Areas table Actions column that opens a shadcn Dialog with the boundary geometry rendered over an Esri World Imagery satellite base layer using Leaflet via react-leaflet 5. Auto-fits the viewport to the boundary on open with 16px padding; scroll-wheel zoom enabled for funder review use. Architecture: the Leaflet island (`area-boundary-map.tsx`) is dynamic-imported with ssr:false inside the dialog wrapper (`preview-area-boundary-dialog.tsx`) because Leaflet imports `window` at module-load and crashes Node (the same react-leaflet gotcha already documented from 6.1b/6.2b-ii). This keeps page.tsx fully SSR-safe — only the dialog body activates Leaflet, and only when previewTarget is non-null. The pure `geojsonToLeafletPositions` helper (colocated at `apps/web/src/app/(dashboard)/patrol-areas/lib/`) handles the [lng,lat]→[lat,lng] coordinate flip + Polygon/LineString shape validation (matching the Prisma GeometryType enum — MultiPolygon is NOT supported by the spec), returning null on malformed input which surfaces as an inline fallback message rather than crashing the dialog. Test strategy: helper has its own pure-function vitest coverage; dialog wrapper is jsdom-tested with the map mocked to a stub component; the Leaflet island itself is NOT unit-tested (vitest jsdom cannot render canvas) — relies on manual visual QA + Leaflet's own contract. vitest count 518 → 535 (+17: +1 table onPreview callback + 8 helper cases + 8 dialog cases). Typecheck + lint + full web suite all green. Cross-package regression: jobs 122/122 + storage 15/15 + shared 154/154 unaffected. Implementation deviations from the plan that were accepted: (1) removed `LatLngBoundsExpression` cast in area-boundary-map.tsx because react-leaflet 5's types already accept `[number, number][]` (lint rule no-unnecessary-type-assertion flagged it); (2) substituted `toBeInTheDocument` with `toBeTruthy`/`toBeNull` because the project doesn't have @testing-library/jest-dom setup; (3) added `afterEach(cleanup)` to the preview dialog test to prevent Radix portal accumulation (matches existing edit-area-boundary-dialog.test.tsx convention).

## 2026-05-24 — 🟢 Area Boundary Management UI Sub-batch A.2 shipped — Create + Edit dialogs with raw GeoJSON validation on /patrol-areas
- Type:      🟢 change
- Phase:     Locked Task Queue 2026-05-23 → Sub-batch A.2 (Area Boundary Management UI: Create + Edit dialogs). Closes the queue. Map preview, map drawing, and 5.1d Area A remain deferred per queue.
- Files:     apps/web/src/app/(dashboard)/patrol-areas/create-area-boundary-dialog.tsx NEW (full form: name, region, aliases CSV, source select, geometryType select, geometry textarea, isEnabled+overrideOfficial switches, arcgisReferenceId; client-side GeoJSON shape validator); apps/web/src/app/(dashboard)/patrol-areas/edit-area-boundary-dialog.tsx NEW (pre-fills from initialBoundary prop; locks source/geometryType/geometryGeojson as read-only disabled inputs + read-only textarea; submits partial diff only); apps/web/src/app/(dashboard)/patrol-areas/area-boundary-table.tsx EDITED (added `onEdit: (boundary) => void` prop; flipped Edit button from disabled stub to clickable handler); apps/web/src/app/(dashboard)/patrol-areas/page.tsx EDITED (replaced Create stub with `showCreate` state + CreateAreaBoundaryDialog; added `editTarget` state + EditAreaBoundaryDialog at root, mirroring deleteTarget pattern); apps/web/src/app/(dashboard)/patrol-areas/__tests__/area-boundary-table.test.tsx EDITED (added onEdit prop to all renders; flipped disabled-Edit assertion to clickable-Edit; +1 onEdit-callback test); apps/web/src/app/(dashboard)/patrol-areas/__tests__/create-area-boundary-dialog.test.tsx NEW (15 cases — field rendering, defaults, trim+null arcgis, comma-aliases parsing, JSON parse failure, Polygon shape rejection on flat coords, LineString shape rejection on nested coords, LineString happy path, blank-name+blank-region rejection, success/error/cancel/pending flows); apps/web/src/app/(dashboard)/patrol-areas/__tests__/edit-area-boundary-dialog.test.tsx NEW (14 cases — pre-fill, locked-fields readonly+disabled assertions, no-changes block, single-field partial update, multi-field partial update with skipped unchanged, aliases-as-array, arcgis-cleared-to-null, blank-name rejection, success+invalidate+plural copy, no-rows-updated copy, error message, cancel, pending-disables-both).
- Concepts:  area-boundary-management, create-area-boundary-dialog, edit-area-boundary-dialog, raw-geojson-textarea, client-side-geojson-shape-validation, polygon-vs-linestring-coords, partial-update-diff, locked-geometry-on-edit, switch-defaults, comma-separated-aliases-parser, arcgis-nullable-trim, sub-batch-a2, locked-task-queue-close, restrict-template-expressions-number-coercion
- Narrative: A.2 sub-batch closes the 2026-05-23 locked queue. Wires the two A.1 disabled stubs (header "Create Area" + per-row "Edit") to full dialog forms that hit `trpc.areaBoundary.create` / `update`. Each create/update server-side fans out an area-rederive job to every Event+Patrol+FuelEntry in the tenant (existing 5.1d helper, unchanged). Client-side GeoJSON validation rejects bad JSON and validates `coordinates` shape against the declared `geometryType` (Polygon → array-of-rings-of-[lng,lat], LineString → array-of-[lng,lat]). Edit dialog locks source/geometryType/geometryGeojson because changing geometry would invalidate the area-derive algorithm contract — admin deletes + recreates if geometry must change. Edit dialog also computes a diff against the initial boundary and submits ONLY changed fields via the existing `updateAreaBoundarySchema.partial()` server schema — server uses `Object.fromEntries(...filter undefined)` so undefined fields are skipped. Discovered + fixed during validation: `@typescript-eslint/restrict-template-expressions` rejects `${number}` in template literals — coerced with `String(feedback.enqueued)` in edit-dialog success copy (delete-dialog avoided this by using JSX siblings instead of template literals). UX choices: comma-separated aliases input (low-friction, no chip-input dep); raw GeoJSON textarea (per locked queue decision — map drawing is deferred); pre-filled success-state Close button (data-testid="create-success-close" / "edit-success-close") to match the A.1 pattern for shadcn Dialog sr-only Close disambiguation; readonly+disabled on Input for locked fields (HTMLInputElement.disabled === true assertion in tests). vitest count 488 → 518 (+30: 1 onEdit table test + 15 create + 14 edit). Typecheck + lint + full web suite all green. Cross-package regression: jobs 122/122 + storage 15/15 + shared 154/154 unaffected. Queue completion gate: A.1 + A.2 both `[x]` → delete .cline/next-tasks.md on commit.

## 2026-05-24 — 🟢 Area Boundary Management UI Sub-batch A.1 shipped — List + Delete + relocated rebuild button on /patrol-areas
- Type:      🟢 change
- Phase:     Locked Task Queue 2026-05-23 → Sub-batch A.1 (Area Boundary Management UI foundation). A.2 (Create + Edit dialogs) remains in `.cline/next-tasks.md` ⏳ — awaits user trigger.
- Files:     apps/web/src/server/trpc/routers/areaBoundary.ts (+2 lines — `source` filter added to `list` input + where clause, mirrors existing `isEnabled` pattern); apps/web/src/app/(dashboard)/patrol-areas/page.tsx (rewritten — client component, useSession-gated admin role check, header with disabled Create stub + RebuildAreaBoundariesButton, body = AreaBoundaryTable + DeleteAreaBoundaryDialog at root); apps/web/src/app/(dashboard)/patrol-areas/area-boundary-table.tsx NEW (debounced region input + isEnabled tri-state + source tri-state filters, cursor pagination matching users-page pattern, role-gated Actions column with Edit-disabled-stub + Delete buttons); apps/web/src/app/(dashboard)/patrol-areas/delete-area-boundary-dialog.tsx NEW (fan-out warning copy, mutation invalidates list query on success, dedicated data-testid="delete-success-close" to disambiguate from shadcn Dialog's sr-only Close button); apps/web/src/app/(dashboard)/patrol-areas/__tests__/area-boundary-table.test.tsx NEW (14 cases); apps/web/src/app/(dashboard)/patrol-areas/__tests__/delete-area-boundary-dialog.test.tsx NEW (11 cases).
- Concepts:  area-boundary-management, patrol-areas-page, area-boundary-table, delete-area-boundary-dialog, rebuild-button-relocation, role-gated-actions, cursor-pagination, debounced-region-filter, source-filter-server-side, shadcn-dialog-srOnly-close-conflict, vitest-jsdom, locked-task-queue, sub-batch-a1, fan-out-rederive-warning, A.2-stub-tooltip
- Narrative: A.1 sub-batch from the 2026-05-23 locked queue (Area Boundary Management UI) — operationalizes the previously-stubbed `/patrol-areas` placeholder into a working management surface. Operators see read-only filtered table; admins see same table plus per-row Edit (disabled stub with "Available in A.2" tooltip) + Delete (functional) + header Create stub (disabled, same tooltip) + relocated RebuildAreaBoundariesButton (was inline in placeholder body; now in page header alongside Create). Locked queue decision: expand `/patrol-areas` rather than create separate `/admin/area-boundaries` — single sidebar entry, conditional admin UI per role. Server-side enhancement: added `source: z.enum(["official","custom"]).optional()` filter to `areaBoundary.list` router input + where clause — 2-line additive mirror of existing `isEnabled` filter; keeps client filter chip wiring simple and pagination correct (vs. client-side post-filter which would yield short pages when source filter is applied). Test isolation: table component takes `isAdmin` as a prop (page derives from useSession) so the table test doesn't need to mock next-auth — only mocks `trpc.areaBoundary.list.useQuery`. Dialog test mocks `trpc.areaBoundary.delete.useMutation` + `trpc.useUtils` with hoisted stubs and a `nextOutcome` switch so individual cases can stage success/error mid-render. Two lint+test fixes discovered during validation: (a) `vi.fn()` for onDelete needs `vi.fn<(b: AreaBoundaryRow) => void>()` type so `.mock.calls[0]?.[0]?.id` survives `@typescript-eslint/no-unsafe-member-access` (matches existing `vi.fn<(input: unknown) => void>()` pattern in users page test); (b) shadcn Dialog renders its own sr-only `Close` button — `getByText("Close")` matches multiple — use `data-testid="delete-success-close"` on the success-state close button. Also: "Enabled"/"Disabled" badge text collides with the enabled filter `<option>` text — use `getAllByText` with `.length >= 1` assertion (mirrors users-page "Active"/"Inactive" handling). vitest count 463 → 488 (+25: 14 table + 11 dialog). Typecheck + lint + full web suite all green. A.2 sub-batch (Create + Edit dialogs with raw GeoJSON validation) wires the disabled stubs to functional dialogs — separate session per queue rule "ship as 2 commits".
...
## 2026-05-30 — 🟤 Impersonation cookie is the correct tenant-switching mechanism for super_admin (not Auth.js JWT update, not URL prefix)
- Type:      🟤 decision
- Phase:     Phase 7 — PRODUCT.md §210 Item 4 (Super_admin tenant context switching)
- Files:     apps/web/src/lib/auth/impersonation.ts, apps/web/src/server/trpc/routers/platformImpersonation.ts, apps/web/src/server/trpc/context.ts, apps/web/src/server/trpc/trpc.ts, apps/web/src/middleware.ts
- Concepts:  impersonation, super_admin, tenant-switching, cookie-based, http-only, samesite-lax, platform-admin, protected-procedure, effective-tenantid, edge-middleware, audit-log
- Narrative: Locked the impersonation mechanism for super_admin tenant context switching as a HttpOnly cookie (`mg-impersonate-tenant`) rather than Auth.js JWT update or URL-prefix routing. Rationale: zero schema change, zero auth-config disruption, edge-safe (middleware reads cookie directly without importing app code), and clean rollback path (delete cookie = exit). Cookie value is the raw tenant cuid; security comes from session + role checks at protectedProcedure layer + write-path validation in enter(), not from the cookie itself. Override happens at protectedProcedure (effectiveTenantId computed once per request when super_admin + empty sessionTenantId + cookie present) — keeps tenant-scoped resolvers and L6 Prisma guard unchanged. Forged cookie by non-super-admin user is ignored because role/tenantId gates fail. URL-prefix routing (/[tenant]/*) per PRODUCT.md §330 spec was REJECTED for Item 4 scope — would require restructuring every (dashboard) page and link. Auth.js JWT update was REJECTED because Auth.js v5 update() is fragile and re-issues the token (high blast radius into auth.config.ts). 24h MaxAge auto-expires the cookie as a safety backstop. AuditLog records both enter and exit with PLATFORM:* prefix and tenantId=null, mirroring Item 3 platformUser convention.

## 2026-05-30 — 🟤 platformImpersonation uses protectedProcedure (not platformAdminProcedure) — exit must work while impersonating
- Type:      🟤 decision
- Phase:     Phase 7 — PRODUCT.md §210 Item 4
- Files:     apps/web/src/server/trpc/routers/platformImpersonation.ts (enter + exit gates)
- Concepts:  platform-admin-procedure, protected-procedure, super_admin, exit-procedure-gate, tenant-context-override
- Narrative: platformImpersonation.enter and platformImpersonation.exit both use `protectedProcedure` directly (NOT platformAdminProcedure). Reason: platformAdminProcedure requires `ctx.tenantId === ""` — but exit() must be callable WHILE impersonating, by which time the ctx.tenantId has been overridden by effectiveTenantId logic to the impersonated tenant's id. Inline gate inside each procedure checks `ctx.session.user.roles.includes("super_admin")` (using raw session.user, not effective ctx) so exit works regardless of impersonation state. enter() additionally checks `ctx.session.user.tenantId === ""` (raw session, the unmarshaled platform-admin signal) to prevent already-tenanted users from impersonating. Pattern is the inverse of Item 3 platformUser router (which can safely use platformAdminProcedure because platformUser procedures are always called from /admin while NOT impersonating). General rule for future platform routers: use platformAdminProcedure when the procedure ONLY makes sense in platform context; use protectedProcedure + inline gates when the procedure must work both in platform context AND while impersonating.

## 2026-05-30 — 🟡 requireRouteAuth() does not read impersonation cookie — SSE + exports 401 during super_admin impersonation
- Type:      🟡 fix
- Phase:     Phase 7 Item 4 Visual QA
- Files:     apps/web/src/server/lib/route-auth.ts, apps/web/src/app/api/stream/notifications/route.ts, apps/web/src/app/api/exports/*/route.ts (all Route Handlers using requireRouteAuth)
- Concepts:  route-handler, requireRouteAuth, sse, impersonation-cookie, super-admin, effective-tenantid, item-4-followup
- Narrative: Item 4 impersonation cookie override happens at `protectedProcedure` layer in apps/web/src/server/trpc/trpc.ts — tRPC procedures get `effectiveTenantId` for super_admin in platform context with cookie. BUT Route Handlers (SSE at /api/stream/notifications, exports at /api/exports/*) bypass tRPC and call `requireRouteAuth()` from apps/web/src/server/lib/route-auth.ts which reads `session.user.tenantId` directly. For impersonating super_admin: raw session tenantId='' so requireRouteAuth() throws 401. Visual QA on 2026-05-30 saw 4× /api/stream/notifications 401s during impersonation flow. Fix pattern: extend requireRouteAuth() to read mg-impersonate-tenant cookie via parseImpersonationCookieFromHeader, mirror the protectedProcedure effective-tenantid logic (super_admin + sessionTenantId='' + cookie present → override). Required before declaring §210 Item 4 fully closed if impersonating super_admin needs SSE notifications or CSV/PDF exports. Architectural note for future platform routers: any code path that authenticates outside tRPC must explicitly handle the impersonation cookie OR document the limitation in DECISIONS_LOG.

## 2026-05-30 — 🟤 Dev compose bakes source at build time — rebuild required after every source change
- Type:      🟤 decision
- Phase:     Phase 7 Item 4 dev-environment polish
