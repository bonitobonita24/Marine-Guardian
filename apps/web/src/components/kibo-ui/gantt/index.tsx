...
export type GanttColumnProps = {
  index: number;
  isColumnSecondary?: ((item: number) => boolean) | undefined;
};
...
export type GanttFeatureItemProps = GanttFeature & {
  onMove?: ((id: string, startDate: Date, endDate: Date | null) => void) | undefined;
...
export const GanttFeatureRow: FC<GanttFeatureRowProps> = ({
  ...
  for (const feature of sortedFeatures) {
    ...
    while (
      ...
      (subRowEndTimes[subRow] ?? new Date(0)) > feature.startAt
    ) {
      subRow++;
    }
