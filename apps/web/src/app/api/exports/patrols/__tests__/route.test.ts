...
const mockedFindMany = prisma.patrol.findMany as unknown as MockFn;
...
function makePatrol(id: string, overrides: Record<string, unknown> = {}) {
  return {
    id,
    serialNumber: `PAT-${id}`,
    title: `Patrol ${id}`,
    ...
    endTime: new Date("2026-05-01T04:00:00Z"),
...
describe("GET /api/exports/patrols", () => {
  it("returns CSV with BOM, text/csv content-type, and tenant-scoped findMany", async () => {
    mockedAuth.mockResolvedValue(authedSession());
    mockedFindMany.mockResolvedValue([makePatrol("p1"), makePatrol("p2")]);

    const req = new NextRequest(
      "http://localhost/api/exports/patrols?format=csv",
    );
    ...
    expect(bodyText).toContain("Patrol p1");
  ...
  it("returns PDF with application/pdf content-type", async () => {
    mockedAuth.mockResolvedValue(authedSession());
    mockedFindMany.mockResolvedValue([makePatrol("p1")]);

    const req = new NextRequest(
      "http://localhost/api/exports/patrols?format=pdf",
    );
    const res = await GET(req);
  ...
  it("writes a DATA_EXPORT AuditLog row with sha256 filterHash after success", async () => {
    ...
    const req = new NextRequest(
      "http://localhost/api/exports/patrols?format=csv&state=open",
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
    ...
    expect(arg?.data.entityType).toBe("patrols");
    expect(arg?.data.entityId).toMatch(/^[a-f0-9]{64}$/);
    expect(arg?.data.changesJson).toEqual({ format: "csv", rowCount: 3 });
  });

  it("propagates state and patrolType filters into prisma.where", async () => {
    mockedAuth.mockResolvedValue(authedSession());
    mockedFindMany.mockResolvedValue([]);

    const req = new NextRequest(
      "http://localhost/api/exports/patrols?format=csv&state=open&patrolType=seaborne",
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
