...
const mockedFindMany = prisma.alertHistory.findMany as unknown as MockFn;
...
function makeHistory(id: string, overrides: Record<string, unknown> = {}) {
  return {
    id,
    alertRuleId: "ar1",
    eventId: "e1",
    firedAt: new Date("2026-05-02T00:00:00Z"),
    matchedPriority: 220,
    recipientCount: 3,
    ruleNameSnapshot: "Priority 200+",
    eventTitleSnapshot: "Whale Sighting",
    alertRule: { id: "ar1", name: "Priority 200+ (current)" },
    event: {
      id: "e1",
      title: "Whale Sighting (current)",
      serialNumber: "EV-001",
      state: "active",
    },
    ...overrides,
...
describe("GET /api/exports/alert-history", () => {
  it("returns CSV with BOM, text/csv content-type, and tenant-scoped findMany", async () => {
    ...
    const req = new NextRequest(
      "http://localhost/api/exports/alert-history?format=csv",
    );
    ...
    // Snapshot columns appear (immutable audit-trail values).
    expect(bodyText).toContain("Priority 200+");
    expect(bodyText).toContain("Whale Sighting");
    // matchedPriority appears as a number column.
  ...
  it("returns PDF with application/pdf content-type", async () => {
    mockedAuth.mockResolvedValue(authedSession());
    mockedFindMany.mockResolvedValue([makeHistory("h1")]);

    const req = new NextRequest(
      "http://localhost/api/exports/alert-history?format=pdf",
    );
    const res = await GET(req);
  ...
  it("writes a DATA_EXPORT AuditLog row with sha256 filterHash after success", async () => {
    ...
    const req = new NextRequest(
      "http://localhost/api/exports/alert-history?format=csv&alertRuleId=ar1",
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
    ...
    expect(arg?.data.entityType).toBe("alert-history");
    expect(arg?.data.entityId).toMatch(/^[a-f0-9]{64}$/);
    expect(arg?.data.changesJson).toEqual({ format: "csv", rowCount: 2 });
  });

  it("propagates alertRuleId filter into prisma.where", async () => {
    mockedAuth.mockResolvedValue(authedSession());
    mockedFindMany.mockResolvedValue([]);

    const req = new NextRequest(
      "http://localhost/api/exports/alert-history?format=csv&alertRuleId=ar-xyz",
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
